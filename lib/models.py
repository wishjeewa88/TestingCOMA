        ...
        with tf.name_scope('loss'):
            with tf.name_scope('data_loss'):
                if self.which_loss == "l1":
                    data_loss = tf.losses.absolute_difference(predictions=outputs, labels=labels, weights=self.loss_weights, reduction=tf.losses.Reduction.MEAN)
                else:
                    data_loss = tf.losses.mean_squared_error(predictions=outputs, labels=labels, weights=self.loss_weights, reduction=tf.losses.Reduction.MEAN)
...
class cgcnn(base_model):
    ...
    """
    def __init__(self, L, weight_tensor, F, K, p, M, nv, which_loss, F_0=1, filter='chebyshev5', brelu='b1relu', pool='mpool1',
    ...
    def upool1(self, x, p):
        ...
        if p > 1:
            x = tf.expand_dims(x, 3) # N x M x F x 1
            x = tf.image.resize_images(x, size=[int(M*p), int(Fin)]) #, method=tf.image.ResizeMethod.BICUBIC)  # N x M x F
...
class coma(base_model):
    ...
    def loss(self, outputs, labels, regularization):
        ...
        with tf.name_scope('loss'):
            with tf.name_scope('data_loss'):
                if self.which_loss == "l1":
                    data_loss = tf.losses.absolute_difference(predictions=outputs, labels=labels, reduction=tf.losses.Reduction.MEAN)
                else:
                    data_loss = tf.losses.mean_squared_error(predictions=outputs, labels=labels, reduction=tf.losses.Reduction.MEAN)
                #cross_entropy = tf.reduce_mean(cross_entropy)
            with tf.name_scope('regularization'):
                regularization *= tf.add_n(self.regularizers)
            loss = data_loss + regularization
